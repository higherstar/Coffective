...
export const validate = () => (dispatch, getState) => {
  const { firstName, lastName, zip, zipError, zipDetails } = getState().register
...
export const validateZip = () => (dispatch, getState) => {
  const { zip, zipError } = getState().register
  dispatch({ type: VALIDATE_ZIP, zipError: zipError || !!validZip(zip) })
}
